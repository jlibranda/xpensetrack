...
export default function ResetPasswordPage() {
  ...
  // Inner content depends on token/success state. NOTE: the page shell is written
  // inline (not as a nested component) so inputs don't lose focus while typing.
  let inner;
  if (!token) {
    inner = (
      <div className="rounded-2xl p-6 text-center" style={{ backgroundColor: cardBg, border: `1px solid ${cardBorder}` }}>
        <p className="text-sm font-medium mb-2" style={{ color: textMain }}>Invalid reset link.</p>
        <Link to="/forgot-password" className="text-sm hover:underline" style={{ color: bg }}>Request a new one →</Link>
      </div>
    );
  } else if (success) {
    inner = (
      <div className="rounded-2xl p-6 text-center" style={{ backgroundColor: cardBg, border: `1px solid ${cardBorder}` }}>
        ...
        <p className="text-sm font-medium" style={{ color: textMain }}>Password reset successfully!</p>
        <p className="text-xs mt-2" style={{ color: textSub }}>Redirecting to sign in...</p>
      </div>
    );
  } else {
    inner = (
    ...
    <div className="min-h-screen flex items-center justify-center p-4 relative"
      ...
      <div className="w-full max-w-sm relative z-10">
        <div className="text-center mb-8">
          ...
          <h1 className="text-xl font-medium" style={{ color: headTitle }}>Set new password</h1>
        </div>
        {inner}
      </div>
    </div>
